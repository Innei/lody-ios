...
The live-activity case runs via `--case live-activity` against the fixture activity in workspace `debug`; no OneSignal, cloud access or credentials are involved, and the settings toggle row is driven through an injected `liveActivity` service so enabling or disabling it never ends the fixture. iOS hides a Live Activity from the Dynamic Island while its own app is in the foreground, so each island capture backgrounds the app first: `island-running` and `island-permission` show the compact island over the Home screen, and `lockscreen-permission` locks the device and captures the Lock Screen card. The Lock Screen capture also contains the one-time system "Allow Live Activities" alert, because the pool leases a clean Simulator for every run. Push-to-start, remote updates and dismissal timing are not covered here and need a real device.
