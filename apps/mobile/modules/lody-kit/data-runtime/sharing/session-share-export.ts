...
export async function prepareSharePackage(options: {
  ...
  if (
    !options.conversations.length ||
    options.conversations.length > SHARE_LIMITS.conversations
  ) {
  ...
  const captured = options.conversations.map((source) => {
    if (selected.has(source.sourceId))
      throw new Error('Duplicate share selection');
    ...
    const bytes = encodeShareJson(
      captureShareHistory(source.history),
      SHARE_LIMITS.historyBytes,
    );
    ...
    const history = validateShareHistory(
  ...
  if (!selected.has(options.rootSourceId))
    throw new Error('Share root not selected');
  ...
  const copied = new Map<
    string,
    {
      id: string;
      source: ShareAttachmentSource;
      references: Record<string, ShareJson>[];
    }
  >();
  let totalBytes = 0;
  async function addObject(
    ...
    if (bytes.length > SHARE_LIMITS.objectBytes)
  ...
  function copyAttachment(
    ...
    if (typeof sourceObjectId !== 'string')
    ...
    if (!copy) {
      if (copied.size >= SHARE_LIMITS.attachments)
      ...
      attachments.push({
        ...
        kind,
        fileName:
          typeof reference.fileName === 'string' ? reference.fileName : id,
        objectId: id,
      });
    }
  ...
  function visit(value: ShareJson, sourceId: string): void {
    ...
    if (
      ...
      value.type = 'text';
      value.text =
        options.fileAttachmentOmissionText ??
  ...
  await mapShareConcurrent(
    ...
    async (copy, _index, signal) => {
      ...
      const descriptor = await addObject(
        copy.id,
        loaded.bytes,
        loaded.mediaType,
      );
      for (const reference of copy.references) {
        ...
        reference[copy.source.kind === 'image' ? 'imageId' : 'fileId'] =
          copy.id;
  ...
  const histories = await mapShareConcurrent(
    ...
    async (source, index, signal) => {
      ...
      const compressed = options.compressHistory
        ? await options.compressHistory(raw)
        : raw;
      ...
      await addObject(
        ...
        'application/json',
        useCompressed
          ? { contentEncoding: 'zstd', decodedSizeBytes: raw.length }
          : undefined,
      );
      const parent =
