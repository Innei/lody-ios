...
export function assertShareAttachmentPolicy(
...
export const SharePackageManifestSchema = z
  .object({
    ...
    conversations: z
      .array(ShareConversationSchema)
      .min(1)
      .max(SHARE_LIMITS.conversations),
  ...
  .superRefine((manifest, ctx) => {
    const invalid = () =>
      ctx.addIssue({ code: 'custom', message: 'Invalid share inventory' });
    const conversations = new Map(
      manifest.conversations.map((entry) => [entry.id, entry]),
    );
    ...
    if (
      ...
      new Set(manifest.attachments.map((entry) => entry.id)).size !==
    ...
    for (const conversation of manifest.conversations) {
      ...
      if (
        conversation.childSessionPlacement &&
        !conversation.parentConversationId
      )
        invalid();
      if (
        conversation.parentConversationId &&
        conversations.get(conversation.parentConversationId)
    ...
    for (const attachment of manifest.attachments) {
      if (
        ...
        objects.get(attachment.objectId)?.contentEncoding
      )
        invalid();
...
export type ShareJson =
  null | boolean | number | string | ShareJson[] | { [key: string]: ShareJson };
...
function assertJson(
  ...
  if (depth > SHARE_LIMITS.jsonDepth)
    throw new Error('Share JSON exceeds depth limit');
  ...
  for (const child of Object.values(value))
    assertJson(child, depth + 1, ancestors);
...
function projectShareHistory(
  ...
  const blocks = (items: ShareJson[]): ShareJson[] =>
    items.flatMap<ShareJson>((item) => {
      ...
      if (item.type === 'system_notice' && item.name === 'task_proposal') {
        if (!omitProposals)
          throw new Error('Task proposals are not allowed in share history');
        return [];
      }
      const result = { ...item };
      if (omitProposals) {
        // Shares retain terminal commands, not live handles or output logs.
        if (item.type === 'terminal' || item.type === 'terminal_output')
          return [];
      ...
      if (item.type === 'tool_call' && Array.isArray(result.content)) {
        ...
        if (
          ...
          if (omitProposals && result.title === command.command)
            delete result.title;
          else if (
            !omitProposals &&
            restoreTitles &&
  ...
  return history.map((entry) => {
    ...
    if (Array.isArray(entry.items))
      result.items = blocks(entry.items) as typeof entry.items;
...
export function captureShareHistory(value: unknown): ShareHistoryEntry[] {
  ...
  return JSON.parse(
    JSON.stringify(projected).replace(
      /#access=v1\.[a-f0-9]{64}/g,
      '#access=omitted',
    ),
...
export async function verifyShareObject(
  bytes: Uint8Array,
  descriptor: ShareObject,
): Promise<void> {
  if (
...
export function shareManifestKey(
  shareId: string,
  deploymentId: string,
): string {
